...
> **Purpose amendment (for archive):** the capability's Purpose currently
> lists `code` among the deferred fields ("no `code`, `imageUrl`,
> `active`, ..."). After this change the entity shape includes `code`
> (10 characters, `NOT NULL UNIQUE`, immutable); `imageUrl`, `active`,
> `lastHydrostaticTestAt` and `hydrostaticTestCount` remain deferred.
...
(Previously: generated only `id` and `deletedAt`; the entity had no
`code`.)
...
(Previously: named only `elementType` and `communityId` as
non-updatable; `code` did not exist.)
