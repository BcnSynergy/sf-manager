...
| E2E (api) | **Two shipped guards must be INVERTED, not left alone** | `apps/api/test/review-history.e2e-spec.ts:1004`'s `'MANAGER gets 403 on both the list and the detail route'` must become `'a granted MANAGER gets 200 []/2xx and an ungranted MANAGER gets 403'`-shaped (still asserting the ungranted case, but no longer asserting a blanket 403 for the role); and `apps/api/test/review-history.e2e-spec.ts:1762`'s `'a caller with a set company but no reviewSession:read gets 403 on every history endpoint (MANAGER variant)'` has no substitute role once all five roles hold `reviewSession:read` and MUST be removed/replaced by the unit-level structural assertion in `specs/authorization/spec.md`'s re-expressed scenario, not patched to use a different role |
