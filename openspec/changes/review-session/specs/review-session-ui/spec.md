...
and community-scope rules by `authorization`. Out of scope: camera / QR
scanning, offline capability of any kind, review history views, due-date
or overdue indicators, signing and export, photos and attachments.
...
The system MUST NOT add offline storage, a service worker, background sync
or conflict resolution; a connection is assumed. It MUST NOT add a review
history view, a due-date or overdue indicator, a signing or export action,
or an attachment/photo control.
...
#### Scenario: No history, scheduling, signing or attachment control exists
- GIVEN every view of the review-session flow
- WHEN its controls are enumerated
- THEN none MUST offer review history, due dates, overdue lists, signing, export, photos or attachments
