...
- [x] 8.1 Add `users.edit.*` i18n keys to `en.json`, `es.json`, `ca.json`.
- [x] 8.2 RED/GREEN `UserEditPage.tsx` + `UserEditPage.test.tsx` — `listUsers()` + filter by `:id` (Decision 5); prefill `email`/`role`, no password field; `role` disabled when `:id === auth.user.id`; not-found state when `:id` absent from the list; `LAST_SYSTEM_ADMIN`/`TRANSACTION_CONFLICT` errors mapped distinctly.
- [x] 8.3 Wire `/users/:id/edit` route into `App.tsx`.
...
- [x] 9.2 Run full web + api lint/test suite; confirm success criteria checklist in `proposal.md`. **Gap found and closed**: the `role` enum value (`SYSTEM_ADMIN`, `MANAGER`, etc.) was rendered raw/untranslated in `UsersListPage.tsx` (table cell) and as `<option>` labels in `UserCreatePage.tsx`/`UserEditPage.tsx` — a real, not-newly-introduced miss against the "Zero hardcoded UI strings" success criterion and the "Internationalization Coverage" spec requirement. Previously flagged as a SUGGESTION in PR6/7/8 reviews and deferred; the user chose to close it now rather than defer further, since this is the last PR before archive. Fixed in this same PR's follow-up commit (`0d7cd70`): `role-labels.ts` maps `Role` to a `users.role.*` i18n key (mirrors `error-messages.ts`'s pattern), new keys added to all 3 locales, all 3 pages updated to render the translated label while `<option value>` and the submitted API payload stay the raw enum. Fresh-context review confirmed no regression to the self-role-guard or submitted values, and the orchestrator browser-verified the full round trip (translated labels render, creating a user via a translated option submits and persists the correct raw role). `proposal.md`'s "Zero hardcoded UI strings" criterion is now checked `[x]` with evidence.
