...
- [x] 6.4 Run full suites: `npm run test --workspace=apps/api`, `npm run test:integration --workspace=apps/api`, `npm run test:e2e --workspace=apps/api`, `npm run test --workspace=apps/web`, `npm run lint`, `npm run build`. All green (773/773 api unit, 108/108 api integration, 295/295 api e2e, 670/670 web, lint clean, build clean).
- [x] 6.5 Browser verification (CLAUDE.md) against a running dev server: log in as `MAINTENANCE_TECHNICIAN` — own history list + drill-in; log in as `COMMUNITY_REPRESENTATIVE` — cross-performer history + drill-in, multi-community row labeling if seeded; deactivate an assignment and re-load to confirm loss of access; empty-state render. All checks passed, including a live confirmation that the PR3 fix (localized answer labels, not raw enum) renders correctly. Found and documented one pre-existing, app-wide (not review-history-specific) gap: raw ISO-8601 timestamps — see design.md.
- [x] 6.6 Note the deferred ADR-011 addendum (design Open Questions — two access services) and the performer-identity / live-element-code gaps as explicit follow-ups; do not author the addendum in this change. Done in design.md's Open Questions section: soft-deleted-community placeholder marked resolved (PR3), the two remaining items carried forward explicitly, plus the new raw-timestamp finding from 6.5 documented as pre-existing app-wide debt (not fixed here, confirmed out of scope).
