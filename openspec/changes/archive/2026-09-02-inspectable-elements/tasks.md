...
- [x] 10.1 `apps/api/test/inspectable-element.e2e-spec.ts` — full lifecycle (create/list/update/delete); element under A never listed under B; create under unknown/soft-deleted community → 404 `COMMUNITY_NOT_FOUND`, no row written; PATCH/DELETE cross-community → 404 `INSPECTABLE_ELEMENT_NOT_FOUND`; two identical name+location accepted; duplicate/absent `serialNumber` accepted; soft-deleted element absent from list. **Deviation from the task brief's "needs real Postgres" note**: built hermetic, in-memory-fake-backed (mirrors every existing e2e spec in this repo — `community`/`maintenance-company`/`users`/`auth` — none use a real DB); the FK/index/parity assertions this brief associated with "real Postgres" are already covered at the integration-spec layer (Phase 6.2/6.3), not e2e.
- [x] 10.2 `apps/api/test/inspectable-element.e2e-spec.ts` — RBAC matrix: 401 unauthenticated / 403 non-admin on all 4 routes, all 4 named non-admin roles (`MANAGER`, `MAINTENANCE_COMPANY_MANAGER`, `MAINTENANCE_TECHNICIAN`, `COMMUNITY_REPRESENTATIVE`) individually; the four non-admin `ROLE_PERMISSIONS` rows still `[]` (spec: authorization).
- [x] 10.3 `apps/api/test/community.e2e-spec.ts` — community delete blocked ⇒ 409 `COMMUNITY_HAS_ACTIVE_ELEMENTS` via a real created element, `deletedAt` stays `null`, no element modified; delete succeeds after soft-deleting every active element; delete succeeds with only soft-deleted elements present (spec: community-management "Soft-Delete Community"). **Also fixed a latent gap in `buildApp`** (not a src change): `INSPECTABLE_ELEMENT_COUNTER` was never overridden, so every existing `DELETE /communities/:id` e2e call was silently 500ing since Phase 3/PR #59 — confirmed via a pre-fix run (`community.e2e-spec.ts`'s "Soft-delete cascade to representative" tests failed with 500 instead of 204/expected). Fixed by adding an `InMemoryInspectableElementCounter` test double wired to the same in-memory element repository.

## Phase 11: Remaining Deltas + Browser Verification + Final Checks (PR 11)
- [x] 11.1 Grep-confirm no `code`, `imageUrl`, `active`, `lastHydrostaticTestAt`, or `hydrostaticTestCount` column, field, or form input exists anywhere (proposal Success Criteria scope guard).
- [x] 11.2 Browser verification (`npm run dev`, `claude-in-chrome`, `SYSTEM_ADMIN` session): create/list/edit/soft-delete an element; blocked-community-delete message; `installedAt` round-trips the same day (07/04/2026 → 2026-07-04); non-admin `NotAuthorized` surface confirmed (renders in place, no redirect, same URL) (CLAUDE.md "Verifying UI Changes"). **Deviation**: es/ca locale spot-check done as content review only (all three locale files' translations already verified real/professional via the `locales.test.ts` parity guard and per-PR review) — this app has no live language switcher yet (`i18n/index.ts` hardcodes `lng: 'en'`; per-user locale is explicitly deferred to ADR-011), so there is no UI mechanism to exercise es/ca live in a browser session.
- [x] 11.3 Full API + web suites, lint, build all pass; `no-restricted-imports` passes; grep confirms no stray `code`/QR/typed-detail artifact exists.
