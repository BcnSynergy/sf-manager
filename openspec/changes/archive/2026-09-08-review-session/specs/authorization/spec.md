...
> "only `SYSTEM_ADMIN` is operational in this slice — the other 4 roles are
> declared and MUST be rejected wherever role-based checks apply". After
> this change that is no longer universally true: `MAINTENANCE_TECHNICIAN`
> and `COMMUNITY_REPRESENTATIVE` become operational **on the review-session
> surface only**, and authorization gains a second, composable dimension —
> a **resource-scope** rule requiring an active community assignment in
> addition to the role/permission check. `MANAGER` and
> `MAINTENANCE_COMPANY_MANAGER` remain fully inert.
...
(Previously: asserted that the other 4 roles' `ROLE_PERMISSIONS` entries
remain `[]`, which no longer holds for the two performing roles.)
...
(Previously: asserted that all four non-admin `ROLE_PERMISSIONS` entries
equal `[]`, which no longer holds for the two performing roles; and said
nothing about decommissioning or by-code resolution.)
...
(Previously: asserted all four non-admin roles map to `[]`.)
