...
> **Purpose amendment (for archive):** the capability's Purpose says "Only
> `MANAGER` remains fully inert — declared but rejected wherever role-based
> checks apply." After this change **no** role is fully inert: `MANAGER`
> holds `reviewSession:read` and becomes the **fifth** role operational on
> the review-history read surface — and the first whose scope is decided
> not by its role alone, nor by an assignment or a company, but by a
> per-user **capability** (`VIEW_ALL_REVIEWS` in
...
> `SYSTEM_ADMIN`'s; ungranted, it reaches nothing. Everything else is
> unchanged: the permission family, the guard order,
> `PermissionChecker.can`'s signature, and the write surface.
...
- WHEN a `SYSTEM_ADMIN` requests review history
- THEN both MUST appear — unlike in any company manager's list, where the unattributed one appears nowhere
...
no grace period and no cached grant. The same immediacy MUST hold for a
revoked `VIEW_ALL_REVIEWS` capability. **Reversed with the product owner
on 2026-09-09:** that rule MUST NOT extend to the caller's own performed
