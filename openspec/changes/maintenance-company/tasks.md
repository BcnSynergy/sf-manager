...
- [x] 13.1 `users.e2e-spec.ts`: shapes 1-3 (missing company, disallowed company, dead/soft-deleted company) with correct codes. **Finding (reported, not fixed — see apply-progress)**: shapes 1 (REQUIRED) and 2 (NOT_ALLOWED) are only reachable through real HTTP with a `code` field when the payload alone cannot decide the violation (PATCH's REQUIRED direction; a PATCH supplying `maintenanceCompanyId` without `role`). For `POST /users` (always) and for `PATCH /users/:id` when `role` is present alongside `maintenanceCompanyId` (the exact combination spec.md's "Missing company for a maintenance role rejected" / "Company id rejected for a non-maintenance role" / "Company id rejected when changing role to a non-maintenance role" scenarios describe), the shared Zod `.superRefine` at `ZodValidationPipe` rejects first with a plain 400 (Zod issues as `message`, no `code`) — the controller/use case, and therefore `UsersController.mapMaintenanceCompanyError`'s `code` assignment, never runs. Verified empirically via a throwaway diagnostic e2e run (not committed) inspecting the raw response bodies. Tests assert the verified-actual behavior (4xx, no user created/modified) for those 3 unreachable-with-code combinations, and assert the full `{statusCode, error, code}` body for the combinations that ARE reachable (shape 3 on both routes; shape 1 via PATCH's resulting-state check; shape 2 via a company-only PATCH on an existing non-maintenance user).
