...
(`/review-templates`), not community-nested. The capability supports
resolving the currently `active` frozen template for an element type,
consumed by `review-session-management`; template authoring itself is
unchanged by that consumption — no new status, no new write path. No
template duplication, no "clone as new draft", no cross-lineage copy, no restore
