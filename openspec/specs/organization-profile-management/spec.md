...
object storage, any consumer of this data (no report rendering, no
branding surface, no "profile must be complete before X" rule), an
...
behaviour in the system. No endpoint, use case or domain rule outside
this capability MUST read the profile or condition itself on the
profile's completeness. Nothing consumes this data in this slice, by
design.

#### Scenario: No other surface consults the profile
- GIVEN the application after this change
- WHEN every read of the organization profile is enumerated
- THEN each MUST originate from this capability's own read endpoint — no other module MUST depend on the profile or on its completeness
