...
permission family and the same active-assignment scope dimension: a
representative may read sessions they did not perform, on their actively
assigned communities only. `MANAGER` and `MAINTENANCE_COMPANY_MANAGER`
remain fully inert — declared but rejected wherever role-based checks
apply, and `SYSTEM_ADMIN` still holds no `reviewSession:*` permission.
Composes with, and runs after, the existing `authentication` guard.
