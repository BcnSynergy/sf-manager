...
after, the existing `authentication` guard. The review-history read
surface now has a **second** endpoint — the element-keyed history read
(`review-history-per-element`) — and the authorization story is
...
which would silently collapse a five-scope feature to one role.
