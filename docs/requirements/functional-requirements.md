...
| FR-010 | Sign and export the completed review as a document, to send to the property management company | **partial** (`review-export`) — slice 1 delivered: conscious signing (**Sign and close**, irreversibility confirmation, signer = performer, signed at = `completedAt`) and a browser-printable review document across all five review-history scopes; slice 2 (signer identity: full name and signature image, snapshotted at signing) is deferred |
