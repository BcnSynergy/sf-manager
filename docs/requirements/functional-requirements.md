...
| FR-008 | View review history, per element and per community — visibility scoped per [ADR-011](../adr/ADR-011-expanded-roles-and-auth-architecture.md): a technician sees only their own sessions, a `MAINTENANCE_COMPANY_MANAGER` sees all of their company's, a community representative sees their community's, `SYSTEM_ADMIN` sees the whole installation, `MANAGER` with `VIEW_ALL_REVIEWS` sees everything | global visibility is half shipped — technician, representative, company and `SYSTEM_ADMIN` installation-wide scopes are all live (`review-history` + `review-history-company-scope` + `review-history-admin-scope`); `MANAGER` + `VIEW_ALL_REVIEWS` (its own `ManagerCapability` mechanism, unbuilt) and per-element history remain the last two pieces — FR-008 does not close until both land |
