...
| FR-008 | View review history, per element and per community — visibility scoped per [ADR-011](../adr/ADR-011-expanded-roles-and-auth-architecture.md): a technician sees only their own sessions, a `MAINTENANCE_COMPANY_MANAGER` sees all of their company's, a community representative sees their community's, `SYSTEM_ADMIN` sees the whole installation, `MANAGER` with `VIEW_ALL_REVIEWS` sees everything | role-based visibility axis is fully shipped — technician, representative, company, `SYSTEM_ADMIN` installation-wide and `MANAGER` + `VIEW_ALL_REVIEWS` (capability-gated, resolved fresh per request) scopes are all live (`review-history` + `review-history-company-scope` + `review-history-admin-scope` + `review-history-manager-capability`); per-element history remains the only deferred piece — FR-008 does not close until it lands |
